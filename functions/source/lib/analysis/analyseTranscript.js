...
const chunkDocuments = (paragraph, chunk_size = CHUNK_SIZE) => {
    paragraph = paragraph.replace("|", "");
    return doChunk(paragraph, chunk_size);
};
const chunks = (paragraph, chunk_size = CHUNK_SIZE) => {
    const textList = [];
...
export const handler = async (event, context) => {
    ...
    try {
        ...
        const [agentEntities, customerEntities, agentKeyPhrases, customerKeyPhrases,] = await Promise
            .all([
        ...
        const analysisResponse = template.replace("<AssetId>", JSON.stringify(assetId))
...
//# sourceMappingURL=analyseTranscript.js.map